...
class Controller(Node):
    ...
    def __init__(self):
        ...
        # Filename distinguishes SINE
        self.csv_filename = os.path.join(self.log_dir, f'log_sine_{timestamp}.csv')
    ...
    def update_trajectory(self):
        # --- Sine Wave Trajectory Configuration ---
        forward_vel = 0.5   
        amp = 1.0           
        omega = 0.5         
        altitude = 2.0      
        ...
        self.pos_des = np.array([
            forward_vel * elapsed,
            amp * math.sin(omega * elapsed),
            altitude
        ])
        
        self.vel_des = np.array([
            forward_vel,
            amp * omega * math.cos(omega * elapsed),
            0.0
        ])
        
        self.acc_des = np.array([
            0.0,
            -amp * (omega**2) * math.sin(omega * elapsed),
            0.0
        ])
    ...
    def print_debug(self, m):
        ...
        self.get_logger().info(
            f"\nCurrently tracking: SINE WAVE Trajectory ({self.flight_duration}s limit)\n"
