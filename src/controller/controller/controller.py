...
class Controller(Node):
    ...
    def __init__(self):
        ...
        # --- Offboard Control Mode (Direct Actuator) ---
        ...
        self.traj_total_time = 0.0
        self.traj_direction = np.zeros(3)
        ...
        self.csv_filename = os.path.join(self.log_dir, f'log_{timestamp}.csv')
        self.csv_file = open(self.csv_filename, mode='w', newline='')
        self.csv_writer = csv.writer(self.csv_file)
        
        # [Updated Header] Added Desired States
        header = [
            'Timestamp', 
            'Pos_X', 'Pos_Y', 'Pos_Z', 
            'Vel_X', 'Vel_Y', 'Vel_Z',
    ...
    def cmdloop_callback(self):
        ...
        if self.mission_complete:
            self.get_logger().info("Target Reached. Closing log and shutting down.")
            self.csv_file.close()
            self.publish_vehicle_command(VehicleCommand.VEHICLE_CMD_COMPONENT_ARM_DISARM, 1.0)
            sys.exit(0) 
    ...
    def log_data(self, throttles):
        ...
        # [Updated Row] Added Desired States
        row = [
            now,
    ...
    def publish_offboard_control_mode(self):
        ...
        msg.thrust_and_torque = False; msg.direct_actuator = True 
    ...
    def init_trajectory(self):
        self.traj_start_pos = self.pos.copy()
        self.traj_start_time = self.get_clock().now().nanoseconds / 1e9
        diff = self.target_position[self.trajectory_state] - self.traj_start_pos
        dist = np.linalg.norm(diff)
        if dist < 0.05: self.traj_total_time = 0.0; self.traj_direction = np.zeros(3)
        else: self.traj_total_time = dist / self.desired_speed; self.traj_direction = diff / dist
        self.is_trajectory_initialized = True

    def update_trajectory(self):
        if not self.is_trajectory_initialized: self.init_trajectory(); return
        now = self.get_clock().now().nanoseconds / 1e9; elapsed = now - self.traj_start_time
        
        if elapsed < self.traj_total_time:
            self.pos_des = self.traj_start_pos + self.traj_direction * self.desired_speed * elapsed
            self.vel_des = self.traj_direction * self.desired_speed
            self.acc_des = np.zeros(3)
        elif elapsed >= self.traj_total_time:
            if self.trajectory_state < len(self.target_position)-1:
                self.trajectory_state += 1
                self.init_trajectory()
            else:
                self.mission_complete = True
                self.pos_des = self.pos
                self.vel_des = np.zeros(3)
                self.acc_des = np.zeros(3)
    ...
    def print_debug(self, m):
        ...
        self.get_logger().info(
            f"\nCurrently tracking target {self.trajectory_state}\n"
